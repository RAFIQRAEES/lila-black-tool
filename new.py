...
def add_event_markers(fig, data, map_id):
    combat_events = {
        "Kill":          ("#ff4444", 22, "✖", "✖ Kill"),
        "Killed":        ("#ff8844", 22, "💀", "💀 Killed"),
        "BotKill":       ("#ff6644", 20, "✖", "✖ Bot Kill"),
        "BotKilled":     ("#ffaa44", 20, "💀", "💀 Bot Killed"),
        "KilledByStorm": ("#aa44ff", 26, "⚡", "⚡ Storm"),
        "Loot":          ("#44aaff", 20, "📦", "📦 Loot"),
    }
    for event_type, (color, size, icon, label) in combat_events.items():
        event_data = data[data['event'] == event_type]
        if event_data.empty:
            continue
        coords = event_data.apply(
            lambda r: world_to_pixel(r['x'], r['z'], map_id), axis=1
        )
...
.stButton > button {
    ...
    height: 60px !important; transition: all 0.3s ease !important;
}
...
with mid_col:
    ...
    st.markdown(
        f"<p style='text-align:center;'>{dots}</p>",
        unsafe_allow_html=True
    )
...
with journey_col:
    if selected_player:
        ...
        label  = (f"🤖 Bot {selected_player}"
                  if is_bot else f"👤 {str(selected_player)[:8]}...")
